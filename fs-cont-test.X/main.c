...
//    char ln[4];
    ...
    char recvbuf[64]; // 受信バッファ（十分なサイズで）
    int counter; // 受信バイト数カウンタ
    int rcvcomp; // 受信完了フラグ
...
//        rcmd[3] = '\0'; 
        ...
        if (SLCT == 0){
            ...
            if (RCIF){
                ...
                if(counter == 0) {
                    if(rcvc == '2') {
                         recvbuf[0] = rcvc; // counterが0のときは'*'しか受け付けない
                             counter++;
                    }
                } else {
                    recvbuf[counter++] = rcvc; // 受信した1文字格納
                    if(counter >= sizeof(recvbuf)) {
                        counter = 0; // 受信バッファが満杯になってしまったらカウンタリセット
                    } else {
